...
| `timestamp` | number | No | Request timestamp for authentication |
...
### Private Streams
Requires API key and signature for user-specific data.
...
const ws = new WebSocket(`wss://ws.gtxdex.xyz?apiKey=${apiKey}&timestamp=${timestamp}&signature=${signature}`);
```
...
# Connect with authentication
ws_url = f"wss://ws.gtxdex.xyz?apiKey={api_key}&timestamp={timestamp}&signature={signature}"
...
Real-time updates for user orders (requires authentication).
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (this.apiKey && this.apiSecret) {
        ...
        connectionUrl += `?apiKey=${this.apiKey}&timestamp=${timestamp}&signature=${signature}`;
      }
...
class GTXWebSocketClient:
    ...
    async def connect(self):
        ...
        if self.api_key and self.api_secret:
            ...
            connection_url += f"?apiKey={self.api_key}&timestamp={timestamp}&signature={signature}"
...
| `4003` | Authentication failed | Check API credentials |
...
<CardGroup cols={2}>
  ...
  >
    Use WebSocket streams with our SDKs
  </Card>
  <Card
    title="API Reference"
    icon="book"
