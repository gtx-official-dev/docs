...
```python Basic Setup - GTX Testnet
import asyncio
from gtx import GTXClient, ChainId

async def main():
    # Initialize the client for GTX Testnet
    client = GTXClient(
...
client = GTXClient(
    chain_id=ChainId.GTX_TESTNET,
    rpc_url=os.getenv("GTX_RPC_URL", "https://testnet-rpc.gtxdex.xyz"),
...
async def run_market_maker():
    client = GTXClient(chain_id=ChainId.GTX_TESTNET)
