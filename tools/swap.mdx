...
|-------|--------|---------|-------------|
| **Wrapped Ethereum** | WETH | RiseLabs | Primary base asset |
| **USD Coin** | USDC | RiseLabs | Stable quote currency |
| **Wrapped Bitcoin** | WBTC | RiseLabs | Bitcoin representation |
| **Chainlink** | LINK | RiseLabs | Oracle token |
| **Pepe** | PEPE | RiseLabs | Meme token |
...
import { GTXClient, OrderSide } from '@gtx/sdk';
...
async function executeSwap() {
  ...
  console.log('Received:', swapResult.outputAmount, 'USDC');
}
```
...
async def execute_swap():
    swap_result = await client.swap(
        from_token="0xWETH_ADDRESS",
        to_token="0xUSDC_ADDRESS",
        amount="1.5",
        slippage_tolerance=0.003,
        recipient=user_address
    )
    
    print(f"Swap completed: {swap_result.transaction_hash}")
    print(f"Received: {swap_result.output_amount} USDC")
```
